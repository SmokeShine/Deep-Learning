...
def content_loss_test(correct, content_loss):
    ...
    bad_img = Variable(torch.zeros(*content_img_var.data.size())).cuda()
    feats = extract_features(bad_img, cnn)

    student_output = content_loss(content_weight, c_feats[content_layer], feats[content_layer]).data.cpu().numpy()
...
def gram_matrix_test(correct, style_loss):
    ...
    student_output = style_loss.gram_matrix(feats[5].clone()).data.cpu().numpy()
    error = rel_error(correct, student_output)
...
def style_loss_test(correct, style_loss):
    ...
    student_output = style_loss(c_feats, style_layers, style_targets, style_weights).data.cpu().numpy()
    error = rel_error(correct, student_output)
...
def tv_loss_test(correct, tv_loss):
    ...
    content_img_var = Variable(content_img.type(dtype)).cuda()

    student_output = tv_loss(content_img_var, tv_weight).data.cpu().numpy()
    error = rel_error(correct, student_output)
