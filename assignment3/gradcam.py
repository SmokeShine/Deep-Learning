...
for i in range(gradcam_result.shape[0]):
    ...
    # https://edstem.org/us/courses/8409/discussion/678159?comment=1676145
    img = rescale(img)
    # img = deprocess(img)
    plt.subplot(1, 5, i + 1)
    plt.imshow(img)
...
# some upsampling is happening
# attribution should be similar as the api might be same

# Computing Guided BackProp
# https://captum.ai/api/guided_backprop.html
# Guided backprop object is not defined
gbp=GuidedBackprop(model)
# same code from saliency
attr_gbp = compute_attributions(gbp, X_tensor, target=y_tensor)
# rename title
visualize_attr_maps('visualization/gbp_captum.png', X, y, class_names, [attr_gbp], ['gbp_captum'])
# not sure what is conv module?

# Computing Guided GradCam
# not sure why doc string ask to calculate gradcam first and then guided backprop
# https://captum.ai/api/guided_grad_cam.html
guided_gc = GuidedGradCam(model,conv_module)
attr_guided_gc = compute_attributions(guided_gc, X_tensor, target=y_tensor)
visualize_attr_maps('visualization/guided_gc_captum.png', X, y, class_names, [attr_guided_gc], ['guided_gc_captum'])
# need to calculate guided backprop, then gradcam and then 
# point wise multiplication of gradcam and guided backprop
...
##############################################################################
layer_gradcam_ = LayerGradCam(model, layer)
attr_layer_gradcam_ = layer_gradcam_.attribute(X_tensor, target=y_tensor,relu_attributions=True)

visualize_attr_maps('visualization/layer_gradcam_captum_.png', 
                    X, y, class_names, [attr_layer_gradcam_], ['layer_gradcam_captum_'])
...
# layer_conductance_attr = layer_conductance.attribute(X_tensor, target=y_tensor)
layer_conductance_attr = compute_attributions(layer_conductance,X_tensor, target=y_tensor)
layer_conductance_attr_sum = layer_conductance_attr.mean(axis=1, keepdim=True)


# layer_act_attr_sum=layer_conductance_attr_sum.repeat(1,3,1,1)
# layer_act_attr_sum[:,1:]=0.
# https://discuss.pytorch.org/t/grayscale-to-rgb-transform/18315
# https://captum.ai/tutorials/Segmentation_Interpret
# from captum.attr import visualization as viz
visualize_attr_maps('visualization/layer_conductance_captum.png', 
                    X, y, class_names, [layer_conductance_attr_sum],
                    ['layer_conductance_captum'],
                    attr_preprocess=lambda attr: attr.permute(1, 2, 0).detach().numpy())
#                     ,
#                     cmap='jet')
# this is random work
# there is gradcam  layergradcam
