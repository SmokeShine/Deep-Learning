...
def train(epoch, data_loader, model, optimizer, criterion):
    ...
    losses = AverageMeter()
    acc = AverageMeter()
    # first cuda call is slow. THis is expected
    for idx, (data, target) in enumerate(data_loader):
        ...
        #############################################################################
        # x and y pair
        # did you watch pytorch tutorial?
        # activate train
        # My BD4H project
        # https://github.com/SmokeShine/Convolutional-Attention-forMultiLabel-classification-CAML/blob/main/src/utils.py
        model.train()
        ...
        # forward pass
        # import pdb;pdb.set_trace()
        # this is commmon code. reshape inside the model
        out=model(data)
        # it requires loss.. that means 2 passes
        # import pdb;pdb.set_trace()
...
def validate(epoch, val_loader, model, criterion):
    ...
    for idx, (data, target) in enumerate(val_loader):
        ...
        #       HINT: torch.no_grad()                                               #
        #############################################################################
        # need to switch train 
        model.eval()
        with torch.no_grad():
            # data is already pushed to device
            # forward pass.. no gradient required
            # 
            out=model(data)
            # need to calculate loss as well
            # optimizer is given
